...
public class UsbCopyManager {
    ...
    public UsbCopyManager(ArgumentManager argumentManager) {
        final String outputFolderArgument = argumentManager.getArgumentValue(ArgumentConfigs.OUTPUT_FOLDER);
        outputFolder = new File(outputFolderArgument == null ? DEFAULT_OUTPUT_FOLDER : outputFolderArgument);
        if (!outputFolder.exists()) {
            outputFolder.mkdirs();
        }

        final String extensionsArgument = argumentManager.getArgumentValue(ArgumentConfigs.EXTENSIONS);
